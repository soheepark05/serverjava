...
<body>
	...
	<p>
	EL은 &lt;%= value %&gt;, out.print(value)와 같이 JSP화면에 표현하고자 하는 코드를
	${value }의 형식으로 표현하여 작성하는 것을 EL이라 한다.
	</p>
	
	<h4>1)EL</h4>
	<a href="el.do">View Details</a> <!-- 404나는 이유 1)페이지없을때 2)서블렛없을때(src부분에넣는다) -->
	
	<h4>2) EL(Param) </h4>
<!-- context path 지정해주는 과정... -->
	<form action ="/03_EL_JSTL/views/el/elParam.jsp" method="post">
	<!-- 위에 아니면 "${ pageContext.request.contextPath}/views/el/elParam.jsp로 쓴다. " -->
		<fieldset>
		 	<legend>제품 입력</legend> 
		 	...
		 	<input type="submit" value="전송"> <!-- 전송누르면 elParam.jsp로 넘어간다. -->
   ...
   <p>
   		jsp페이지에서 자바 코드 등의 스크립트 언어를 사용하지 않고<br>
   		HTML 태그 형식으로 다른 페이지나 객체에 접근할 수 있도록 태그를 이용해 구현한 기능을 말한다.
   </p>

	<h4>1)표현 액션 태그(Standard Action Tag) </h4>
	<p>
	...
	<p>
		...
		커스텀 액션 태그는 모든 태그의 이름앞에 jsp: 이외의 접두어가 붙는다.
	</p>
	...
	<h5>Jstl formatting library</h5>
	<p>
	날짜,시간,숫자 데이터의 출력형식을 지정할때 사용하는 태그를 제공한다.
	</p>
